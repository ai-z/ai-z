  ...
  while (running) {
    ...
    // immediately, instead of waiting for the next refresh tick.
    {
      ...
      for (const int k : queuedKeys) {
        ...
        if (cmd == Command::Toggle && state.screen == Screen::Config) {
        ...
        if (cmd == Command::Defaults && state.screen == Screen::Config) {
          cfg = Config{};
          continue;
        }

        if (cmd == Command::Save && state.screen == Screen::Config) {
          cfg.save();
          continue;
        }

        if (cmd == Command::Activate && state.screen == Screen::Config) {
          // Enter only activates the action row to avoid accidental toggles.
          if (state.configSel == 11) cfg = Config{};
          continue;
        }
